...
function M.clickable(str, handler, arg)
	return "%" .. (arg or "") .. "@vy" .. handler .. "@" .. str .. "%X"
end
...
function M.merge_hl(icon_hl, buf_hl)
	local hl_name = "vy_" .. icon_hl .. "_" .. buf_hl
	local fg = api.nvim_get_hl(0, { name = icon_hl, link = false }).fg
	local bg = api.nvim_get_hl(0, { name = "vy" .. buf_hl, link = false }).bg
...
function M.render_tab(tabnr, is_active)
	local tab_hl = is_active and "vyTabOn" or "vyTabOff"
	local buf_hl = is_active and "vyBufOn" or "vyBufOff"
	local merged = "vyTabIn" .. (is_active and "Active" or "Inactive")
